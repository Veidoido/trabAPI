...
function showResult(){
    ...
    let dados = JSON.parse(this.responseText);
    for(i=0;i<3;i++){
        let showResult = dados.items[i];
        let data = new Date (showResult.updated_at);
        string = string + ` <div class="col12 border border-primary rounded-3 retornoPesquisa">
                            <img class="imgpequisa rounded-3";" src="${showResult.owner.avatar_url}">
                                <p><strong>${showResult.name}</strong><br><span style="font-size: small;">${showResult.owner.login}</span></p>
                                <p>${showResult.description}<br><span style="font-size: small;">Última atualização: ${data.toLocaleDateString()}</span></p>
                                <a href="${showResult.html_url}" target="_blank"><button type="button" class="btn btn-sm btn-outline-primary ">Ver Repositório</button></a>
                            </div>`;
    }
